...
export function TicketList({ tickets }: { tickets: TicketRow[] }) {
  ...
  const filtered = useMemo(() => tickets.filter((t) => {
    ...
    const matchSearch = !q || t.folio.toLowerCase().includes(q) ||
      ...
      (t.ticket_categories?.name ?? '').toLowerCase().includes(q)
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <Input
            placeholder="Buscar por folio, descripción o categoría…"
            value={search}
        ...
        <Select value={status} onValueChange={(v) => v !== null && setStatus(v)}>
          <SelectTrigger className="w-full sm:w-40">
            <SelectValue placeholder="Estado">
              {status === 'all' ? 'Todos los estados' : STATUS_LABELS[status as TicketStatus]}
            </SelectValue>
          </SelectTrigger>
          <SelectContent>
        ...
        <Card className="border-dashed border-zinc-200 dark:border-zinc-800">
          <CardContent className="py-12 text-center">
            ...
            <p className="text-sm text-zinc-500">
              {tickets.length === 0 ? 'No tienes tickets aún.' : 'Sin resultados para los filtros aplicados.'}
            </p>
          </CardContent>
        </Card>
      ) : (
                ...
                <CardContent className="py-3 px-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-zinc-400 mt-1">{formatDate(t.created_at)}</p>
                    </div>
