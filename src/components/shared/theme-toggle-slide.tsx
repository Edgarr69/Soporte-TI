...
export function ThemeToggleSlide({ isOpen }: { isOpen: boolean }) {
  ...
  useEffect(() => { if (isOpen) setIsTooltipOpen(false) }, [isOpen])
  ...
  return (
    ...
    >
      <label
        className={`relative flex items-center justify-center cursor-pointer text-xl select-none
      ...
      >
        ...
        <div className={`flex items-center ${isOpen ? 'w-full' : 'w-6 h-6 pl-1 ml-1'}`}>
          ...
          <motion.div
            ...
            animate={isLight ? 'checked' : 'unchecked'}
          ...
          <motion.div
            ...
            variants={toggleVariants}
            animate={isLight ? 'unchecked' : 'checked'}
            transition={transition}
          >
