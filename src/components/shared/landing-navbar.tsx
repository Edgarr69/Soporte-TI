...
function ThemeToggleIcon() {
  ...
  return (
    <div className="flex items-center justify-center relative size-4">
      {/* Moon */}
      <div className={cn(
        'absolute h-full flex items-center transition-all duration-500 ease-in-out',
...
const PILL = 'flex items-center gap-2 py-2 rounded-full border border-white/20 bg-background/60 px-2 shadow-md backdrop-blur-sm backdrop-saturate-150 dark:bg-zinc-900/50'
  ...
  return (
    <>
      <nav className="w-full py-2 flex justify-center gap-3 bg-transparent">
        ...
        <div className={cn(PILL, 'gap-1')}>
          <button
            onClick={() => setIsLoginOpen(true)}
            className="flex items-center gap-1.5 text-sm px-2 py-1.5 rounded-xl hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors text-foreground"
          >
            <LogIn size={15} />
